...
PROVIDER_RETRIES = max(0, int(os.getenv('PROVIDER_RETRIES', '2')))
PROVIDER_RETRY_BACKOFF = max(1, int(os.getenv('PROVIDER_RETRY_BACKOFF', '3')))
MAX_TOKENS = min(16384, max(256, int(os.getenv('PROVIDER_MAX_TOKENS', '16384'))))
REASONING_EFFORT = os.getenv('REASONING_EFFORT', 'high')
SHELL_ENABLED = os.getenv('ENABLE_SHELL', '1').lower() not in {'0', 'false', 'no'}
SHELL_TIMEOUT = max(0, int(os.getenv('SHELL_TIMEOUT', '0')))
SESSION_TTL = max(300, int(os.getenv('SESSION_TTL', '86400')))
MAX_AGENT_STEPS = max(0, int(os.getenv('MAX_AGENT_STEPS', '0')))
JOB_RETENTION = max(300, int(os.getenv('JOB_RETENTION', '86400')))
...
def load():
        ...
        try:
            ...
            if isinstance(current, dict):
                for key, value in current.items():
                    if key in {'provider', 'search'} and isinstance(value, dict):
                        merged = dict(d[key])
    ...
    d['provider']['base_url'] = d['provider'].get('base_url') or DEFAULT_BASE_URL
    d['provider']['model'] = d['provider'].get('model') or DEFAULT_MODEL
    d.setdefault('search', {'provider': 'duckduckgo', 'api_key': ''})
    d.setdefault('chats', [])
    d.setdefault('skills', [])
    try:
...
def shell_tool(args):
    ...
    if isinstance(command, list):
        ...
        display = str(command or '').strip()
        if not display:
            raise ValueError('Missing command')
        argv = ['/bin/sh', '-lc', display]
    kwargs = {'cwd': str(WORKSPACE), 'capture_output': True, 'text': True}
    if SHELL_TIMEOUT > 0:
        kwargs['timeout'] = SHELL_TIMEOUT
    proc = subprocess.run(argv, **kwargs)
    return {'ok': proc.returncode == 0, 'command': display, 'exit_code': proc.returncode, 'stdout': proc.stdout[-12000:], 'stderr': proc.stderr[-12000:]}
...
def search_web(query, provider='duckduckgo', api_key=''):
    ...
    if (provider or '').lower() == 'brave' and api_key:
    ...
    req = urllib.request.Request(url, headers={'User-Agent': 'JolgueAI/2.1'})
    with urllib.request.urlopen(req) as r:
    ...
    for i, (url0, title0) in enumerate(blocks[:8]):
        title = html.unescape(re.sub(r'\s+', ' ', re.sub(r'<.*?>', '', title0))).strip()
        snippet = html.unescape(re.sub(r'\s+', ' ', re.sub(r'<.*?>', ' ', snippets[i]))).strip() if i < len(snippets) else ''
        out.append({'title': title, 'url': html.unescape(url0), 'snippet': snippet})
    return out
...
def stream_provider(req, job_id):
    set_job(job_id, phase='provider', provider_status='connecting', chunks=0)
    with urllib.request.urlopen(req) as response:
        ...
        for raw_line in response:
            ...
            if piece:
                ...
                chunks += 1
                set_job(job_id, provider_status='streaming', chunks=chunks)
...
Emit each tool action exactly as one line:
<tool>{"name":"TOOL_NAME","args":{...}}</tool>

Rules:
- Use workspace-relative paths only.
- Use shell for builds, tests, inspection and running programs.
- Use file tools for file operations.
- Use web_search for current or external information.
- Inspect every tool result and continue when needed.
- Do not claim an action happened unless the tool result confirms it.
- For large projects, work incrementally: inspect -> implement -> test -> fix -> continue.
- Keep each individual response focused enough to make progress; do not dump an entire huge project in one response.
'''
    ...
    for attempt in range(PROVIDER_RETRIES + 1):
        try:
            value = stream_provider(req, job_id)
            try:
                return value['choices'][0]['message']['content']
            except (KeyError, IndexError, TypeError):
                raise RuntimeError('Provider returned an unexpected response: ' + json.dumps(value, ensure_ascii=False)[:4000])
        except urllib.error.HTTPError as exc:
            last_error = exc
            if exc.code != 504 or attempt >= PROVIDER_RETRIES:
                try:
                    detail = exc.read().decode(errors='ignore')[:6000]
                except Exception:
                    detail = str(exc)
                raise RuntimeError(f'Provider HTTP {exc.code}: {detail}')
            set_job(job_id, phase='retrying', provider_status=f'504 received; retry {attempt + 1}/{PROVIDER_RETRIES}')
            time.sleep(PROVIDER_RETRY_BACKOFF * (attempt + 1))
...
def execute_tool_calls(text, search_cfg):
    ...
    return pattern.sub('', text).strip(), results
...
def agent_run(payload, job_id):
    ...
    while True:
        ...
        if job_cancelled(job_id):
            raise RuntimeError('Generation stopped')
        step += 1
        set_job(job_id, step=step, phase='thinking', last_tools=[])
        turn = dict(payload)
        ...
        set_job(job_id, phase='tools', tools=all_tools, last_tools=tools)
...
def active_jobs_for(username):
    with JOBS_LOCK:
        return [{'job_id': jid, 'status': job.get('status'), 'phase': job.get('phase'), 'step': job.get('step', 0), 'created': job.get('created'), 'updated': job.get('updated')} for jid, job in JOBS.items() if job.get('username') == username and job.get('status') == 'running']
...
def job_public(job):
    ...
    if job.get('status') == 'completed':
        ...
        out['content'] = 'Generation stopped.'
    return out
...
class H(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        cleanup_jobs()
        if self.path == '/health':
            self.send_json({'ok': True, 'service': 'jolgue-ai', 'port': PORT, 'base_url': DEFAULT_BASE_URL, 'model': DEFAULT_MODEL})
            return
        if self.path == '/logout':
            c = SimpleCookie()
            try:
        ...
        if self.path == '/':
            if not require_auth(self):
                ...
                return
            self.send_html((TEMPLATES / 'chat.html').read_text(encoding='utf-8'))
            return
        if not require_auth(self):
            self.unauthorized()
            return
        ...
        if self.path.startswith('/api/jobs/'):
            job_id = urllib.parse.unquote(self.path.split('/api/jobs/', 1)[1].split('?', 1)[0])
            with JOBS_LOCK:
                job = JOBS.get(job_id)
            ...
            if isinstance(data, dict):
                for key, value in data.items():
                    if key == 'provider' and isinstance(value, dict):
                        current['provider'] = {**current.get('provider', {}), **value}
                    elif key == 'search' and isinstance(value, dict):
                        current['search'] = {**current.get('search', {}), **value}
                    else:
                        current[key] = value
...
if __name__ == '__main__':
    ...
    print(f'Provider streaming: on | max_tokens={MAX_TOKENS} | reasoning={REASONING_EFFORT}')
    print(f'Provider 504 retries: {PROVIDER_RETRIES}')
